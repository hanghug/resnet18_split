...
template <class data_T, class res_T, int N, int n_chan>
void clone_stream(hls::stream<data_T> (&data)[n_chan], hls::stream<res_T> (&res1)[n_chan], hls::stream<res_T> (&res2)[n_chan]) {
CloneLoop:
    for (int i = 0; i < N / n_chan; i++) {
        #pragma HLS PIPELINE
        #pragma HLS PIPELINE
        data_T in_data[n_chan];
        res_T out_data1[n_chan];
        res_T out_data2[n_chan];
    ReadLoop:
        for (int c = 0; c < n_chan; c++) {
            #pragma HLS UNROLL
            in_data[c] = data[c].read();
        }
    ClonePack:
        for (int c = 0; c < n_chan; c++) {
            #pragma HLS UNROLL
            out_data1[c] = in_data[c];
            out_data2[c] = in_data[c];
        }
    WriteLoop:
        for (int c = 0; c < n_chan; c++) {
            #pragma HLS UNROLL
            res1[c].write(out_data1[c]);
            res2[c].write(out_data2[c]);
        }
    }
}

template <class data_T, class res_T, int N, int n_chan>
void clone_stream(hls::stream<data_T> (&data)[n_chan/data_T::size], hls::stream<res_T> (&res1)[n_chan/res_T::size], hls::stream<res_T> (&res2)[n_chan/res_T::size]) {
CloneLoop:
    for (int i = 0; i < N / n_chan; i++) {
        #pragma HLS PIPELINE
        #pragma HLS PIPELINE
        typename data_T::value_type data_array[n_chan];
    ReadLoop:
        for (int j = 0; j < n_chan/data_T::size; j++) {
            #pragma HLS UNROLL
            data_T in_data_pack = data[j].read();
            for (int k = 0; k < data_T::size; k++) {
                #pragma HLS UNROLL
                data_array[j*data_T::size + k] = in_data_pack[k];
            }
        }
    WriteLoop:
        for (int j = 0; j < n_chan/res_T::size; j++) {
            ...
            for (int k = 0; k < res_T::size; k++) {
                #pragma HLS UNROLL
                out_data_pack[k] = data_array[j*res_T::size + k];
            }
            res1[j].write(out_data_pack);
            res2[j].write(out_data_pack);
        }
    }
}

template <class data_T, class res_T, int N, int n_chan>
void clone_stream(hls::stream<data_T> (&data)[n_chan], hls::stream<res_T> (&res1)[n_chan], hls::stream<res_T> (&res2)[n_chan], hls::stream<res_T> (&res3)[n_chan]) {
CloneLoop:
    for (int i = 0; i < N / n_chan; i++) {
        #pragma HLS PIPELINE
        data_T in_data[n_chan];
