...
template <class data_T, class res_T, typename CONFIG_T, int n_chan> void relu(hls::stream<data_T> (&data)[n_chan], hls::stream<res_T> (&res)[n_chan]) {
ReLUActLoop:
    for (int i = 0; i < CONFIG_T::n_in / n_chan; i++) {
        #pragma HLS PIPELINE

        data_T in_data [n_chan];
...
template <class data_T, class res_T, typename CONFIG_T, int n_chan> void relu(hls::stream<data_T> (&data)[n_chan/data_T::size], hls::stream<res_T> (&res)[n_chan/res_T::size]) {
ReLUActLoop:
    for (int i = 0; i < CONFIG_T::n_in / n_chan; i++) {
        #pragma HLS PIPELINE

        typename data_T::value_type in_data [n_chan];
        typename res_T::value_type out_data [n_chan];
    ReLUUnpackLoop:
        for (int j = 0; j < n_chan/data_T::size; j++) {
            #pragma HLS UNROLL
            data_T data_pack = data[j].read();
            for (int k = 0; k < data_T::size; k++) {
                #pragma HLS UNROLL
                in_data[j*data_T::size+k] = data_pack[k];
            }
        }
    ReLUPackLoop:
        for (int j = 0; j < n_chan; j++) {
    ...
    ReLUWriteLoop:
        for (int j = 0; j < n_chan/res_T::size; j++) {
            ...
            for (int k = 0; k < res_T::size; k++) {
                #pragma HLS UNROLL
                res_pack[k] = out_data[j*res_T::size+k];
            }
...
template <class data_T, class res_T, typename CONFIG_T>
void softmax_latency(hls::stream<data_T> (&data)[CONFIG_T::n_in], hls::stream<res_T> (&res)[CONFIG_T::n_in]) {
    // Initialize the lookup tables
#ifdef __HLS_SYN__
    bool initialized = false;
    typename CONFIG_T::exp_table_t exp_table[CONFIG_T::table_size];
    ...
    data_T in_pack[CONFIG_T::n_in];
SoftmaxReadLoop:
    for (unsigned j = 0; j < CONFIG_T::n_in; j++) {
        #pragma HLS UNROLL
...
SoftmaxExpPackLoop:
    for (unsigned j = 0; j < CONFIG_T::n_in; j++) {
        #pragma HLS UNROLL
        unsigned x = softmax_idx_from_real_val<data_T, CONFIG_T>(in_pack[j]);
        exp_res[j] = exp_table[x];
    }
...
template <class data_T, class res_T, typename CONFIG_T>
void softmax_stable(hls::stream<data_T> (&data)[CONFIG_T::n_in], hls::stream<res_T> (&res)[CONFIG_T::n_in]) {
    // Initialize the lookup tables
#ifdef __HLS_SYN__
    bool initialized = false;
    typename CONFIG_T::exp_table_t exp_table[CONFIG_T::table_size];
    ...
    data_T data_array[CONFIG_T::n_in];
#pragma HLS ARRAY_PARTITION variable=data_array complete
SoftmaxArrayLoop:
    #pragma HLS PIPELINE II=ii
SoftmaxArrayPackLoop:
    for (unsigned j = 0; j < CONFIG_T::n_in; j++) {
        #pragma HLS UNROLL
        data_array[j] = data[j].read();
    }

    // Find the max and compute all delta(x_i, x_max)
    Op_max<data_T> op_max;
    data_T x_max =
        reduce<data_T, CONFIG_T::n_in, Op_max<data_T>>(data_array, op_max);
    ...
    for (unsigned j = 0; j < CONFIG_T::n_in; j++) {
        #pragma HLS UNROLL
        unsigned x = softmax_idx_from_real_val<data_T, CONFIG_T>(d_xi_xmax[j]);
        exp_res[j] = exp_table[x];
    }
...
SoftmaxInvPackLoop:
    for (unsigned j = 0; j < CONFIG_T::n_in; j++) {
        #pragma HLS UNROLL factor=multiplier_limit
        #pragma HLS ALLOCATION operation instances=mul limit=multiplier_limit
        out_pack[j] = exp_res[j] * inv_exp_sum;
    }
SoftmaxWriteLoop:
...
template <class data_T, class res_T, typename CONFIG_T>
void softmax_legacy(hls::stream<data_T> (&data)[CONFIG_T::n_in], hls::stream<res_T> (&res)[CONFIG_T::n_in]) {
    // Initialize the lookup table
#ifdef __HLS_SYN__
    bool initialized = false;
    typename CONFIG_T::table_t exp_table[CONFIG_T::table_size];
...
SoftmaxInvPackLoop:
    for (unsigned j = 0; j < CONFIG_T::n_in; j++) {
        ...
        out_pack[j] = (res_T)invert_table[exp_res_index];
    }
SoftmaxWriteLoop:
    for (unsigned j = 0; j < CONFIG_T::n_in; j++) {
        #pragma HLS UNROLL
...
template <class data_T, class res_T, typename CONFIG_T>
void softmax_argmax(hls::stream<data_T> (&data)[CONFIG_T::n_in], hls::stream<res_T> (&res)[CONFIG_T::n_in]) {
    #pragma HLS PIPELINE

    data_T in_data[CONFIG_T::n_in];
    res_T out_data[CONFIG_T::n_in];
SoftmaxReadLoop:
    for (unsigned j = 0; j < CONFIG_T::n_in; j++) {
        #pragma HLS UNROLL
        in_data[j] = data[j].read();
...
template <class data_T, class res_T, typename CONFIG_T>
void softmax_stable(hls::stream<data_T> (&data)[CONFIG_T::n_in/data_T::size], hls::stream<res_T> (&res)[CONFIG_T::n_in/res_T::size]) {
    // Initialize the lookup tables
#ifdef __HLS_SYN__
    bool initialized = false;
    typename CONFIG_T::exp_table_t exp_table[CONFIG_T::table_size];
...
SoftmaxArrayLoop:
    #pragma HLS PIPELINE II=ii
SoftmaxArrayPackLoop:
    for (unsigned j = 0; j < CONFIG_T::n_in/data_T::size; j++) {
        #pragma HLS UNROLL
        data_T data_pack = data[j].read();
    ...
    typename res_T::value_type out_array[CONFIG_T::n_in];

SoftmaxInvPackLoop:
    for (unsigned j = 0; j < CONFIG_T::n_in; j++) {
        #pragma HLS UNROLL factor=multiplier_limit
        #pragma HLS ALLOCATION operation instances=mul limit=multiplier_limit
        out_array[j] = exp_res[j] * inv_exp_sum;
    }
SoftmaxWriteLoop:
    for (unsigned j = 0; j < CONFIG_T::n_in/res_T::size; j++) {
        #pragma HLS UNROLL
        res_T out_pack;
        for (unsigned k = 0; k < res_T::size; k++) {
            #pragma HLS UNROLL
            out_pack[k] = out_array[j*res_T::size + k];
        }
        res[j].write(out_pack);
    }
}

template <class data_T, class res_T, typename CONFIG_T> void softmax(hls::stream<data_T> (&data)[CONFIG_T::n_in], hls::stream<res_T> (&res)[CONFIG_T::n_in]) {
    assert(CONFIG_T::axis == -1);
...
template <class data_T, class res_T, typename CONFIG_T> void softmax(hls::stream<data_T> (&data)[CONFIG_T::n_in/data_T::size], hls::stream<res_T> (&res)[CONFIG_T::n_in/res_T::size]) {
    assert(CONFIG_T::axis == -1);
    assert (CONFIG_T::implementation == softmax_implementation::stable && "Only stable implementation is supported for this axis configuration");
