...
#else   //! _Pragma运算符在C和C++中用于作为程序的一部分发出编译器指令 HLS DATA_PACK variable = xxx, 通过_Pragma添加#pragma
//#define PRAGMA_DATA_PACK(variable) _Pragma("#pragma HLS DISAGGREGATE variable=" #variable)
#define PRAGMA_DATA_PACK(variable)
#endif   //!在vitis_hls中不执行操作。vitis__hls中没有数据对齐的DATA_PACK操作了，默认对齐方式，使用DISAGGREGATE解绑结构体，默认在一起的。
