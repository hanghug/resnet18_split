...
template <class data_T, typename CONFIG_T>
void kernel_shift_1d(hls::stream<data_T> (&data_stream)[CONFIG_T::n_chan/data_T::size],
...
KernelShiftWidth:
    ...
    static const int lastheight = (CONFIG_T::filt_width - 1) * CONFIG_T::n_chan;
KernelPushChannel:
    for (int i_ic = 0; i_ic < CONFIG_T::n_chan/data_T::size; i_ic++) {
        #pragma HLS UNROLL
        data_T data_pack = data_stream[i_ic].read();
        for (int i_is = 0; i_is < data_T::size; i_is++) {
            #pragma HLS UNROLL
            kernel_window[lastheight + i_ic*data_T::size + i_is] = data_pack[i_is];
        }
    }
}
...
void shift_line_buffer(
    hls::stream<data_T> (&data_stream)[CONFIG_T::n_chan/data_T::size],
...
UpdateBuffer:
    for (int i_ic = 0; i_ic < CONFIG_T::n_chan/data_T::size; i_ic++) {
        ...
        for (int i_is = 0; i_is < data_T::size; i_is++) {
            #pragma HLS UNROLL
            shift_buffer[CONFIG_T::filt_height - 1][i_ic*data_T::size + i_is] = data_pack[i_is];
        }
...
void compute_output_buffer_2d(
    ...
    ap_shift_reg<typename data_T::value_type, CONFIG_T::in_width> line_buffer[MAX(CONFIG_T::filt_height - 1, 1)]
                                                                             [CONFIG_T::n_chan],
    hls::stream<res_T> (&res_stream)[CONFIG_T::n_filt/res_T::size],
    typename CONFIG_T::weight_t weights[CONFIG_T::kernel_size * CONFIG_T::n_chan * CONFIG_T::n_filt],
    typename CONFIG_T::bias_t biases[CONFIG_T::n_filt]) {
    ...
    PRAGMA_DATA_PACK(res_out)

    // Add pixel to buffer
    nnet::shift_line_buffer<data_T, CONFIG_T>(data_stream, line_buffer, kernel_data);
    ...
    // Pack output
    CastLoop:
        for (unsigned i_ic = 0; i_ic < CONFIG_T::n_filt/res_T::size; i_ic++) {
            #pragma HLS UNROLL
            res_T res_pack;
            for (unsigned i_oc = 0; i_oc < res_T::size; i_oc++) {
                #pragma HLS UNROLL
                res_pack[i_oc] = res_out[i_ic * res_T::size + i_oc];
            }
            res_stream[i_ic].write(res_pack);
        }
...
void compute_output_buffer_1d(
    hls::stream<data_T> (&data_stream)[CONFIG_T::n_chan/data_T::size], hls::stream<res_T> (&res_stream)[CONFIG_T::n_filt/res_T::size],
    typename CONFIG_T::weight_t weights[CONFIG_T::kernel_size * CONFIG_T::n_chan * CONFIG_T::n_filt],
    typename CONFIG_T::bias_t biases[CONFIG_T::n_filt]) {
    #pragma HLS INLINE
    ...
    // Add pixel to buffer
    nnet::kernel_shift_1d<data_T, CONFIG_T>(data_stream, kernel_data);
...
// template <class data_T, typename CONFIG_T>
// void kernel_shift_1d(hls::stream<data_T> (&data_stream)[CONFIG_T::n_chan],
...
//     for (int i_ic = 0; i_ic < CONFIG_T::n_chan; i_ic++) {
//         #pragma HLS UNROLL
//         kernel_window[lastheight + i_ic] = data_stream[i_ic].read();
//     }
// }
...
//     data_T kernel_window[CONFIG_T::filt_width * CONFIG_T::filt_height * CONFIG_T::n_chan]) {
//     #pragma HLS inline
...
//     ap_shift_reg<data_T, CONFIG_T::in_width> line_buffer[MAX(CONFIG_T::filt_height - 1, 1)]
//                                                                              [CONFIG_T::n_chan],
...
//         // Insert pixel(s) at end of shift buffer
//         shift_buffer[CONFIG_T::filt_height - 1][i_ic] = data_stream[i_ic].read();
//     }
...
//         for (unsigned i_ih = 1; i_ih < CONFIG_T::filt_height; i_ih++) {
//             #pragma HLS UNROLL
//             data_T pop_elem = line_buffer[i_ih - 1][i_ic].shift(
//                 shift_buffer[CONFIG_T::filt_height - i_ih][i_ic]); // Shift the line buffer, return the popped pixel
//             shift_buffer[CONFIG_T::filt_height - i_ih - 1][i_ic] =
//                 pop_elem; // Popped element placed back into shift_buffer, one row up.
//         }
...
//         // #pragma HLS INLINE recursive
//         if (CONFIG_T::strategy == nnet::latency) {
//             dense_latency<data_T, res_T, typename CONFIG_T::mult_config>(
//                 kernel_data, res_out, weights, biases);
//         } else {
...
// template <class data_T, class res_T, typename CONFIG_T>
// void compute_output_buffer_1d(
//     hls::stream<data_T> (&data_stream)[CONFIG_T::n_chan], hls::stream<res_T> (&res_stream)[CONFIG_T::n_filt],
//     typename CONFIG_T::weight_t weights[CONFIG_T::kernel_size * CONFIG_T::n_chan * CONFIG_T::n_filt],
//     typename CONFIG_T::bias_t biases[CONFIG_T::n_filt]) {
//     #pragma HLS INLINE
