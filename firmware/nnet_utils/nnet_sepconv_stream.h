...
template <class data_T, class res_T, typename CONFIG_T>
void pointwise_mult_buffer(hls::stream<data_T> (&data_stream)[CONFIG_T::n_chan], hls::stream<res_T> (&res_stream)[CONFIG_T::n_filt],
                           typename CONFIG_T::weight_t weights[CONFIG_T::n_chan * CONFIG_T::n_filt],
                           typename CONFIG_T::bias_t biases[CONFIG_T::n_filt]) {
    #pragma HLS INLINE
...
template <class data_T, class res_T, typename CONFIG_T>
void pointwise_mult_buffer(hls::stream<data_T> (&data_stream)[CONFIG_T::n_chan/data_T::size], hls::stream<res_T> (&res_stream)[CONFIG_T::n_filt/res_T::size],
                           typename CONFIG_T::weight_t weights[CONFIG_T::n_chan * CONFIG_T::n_filt],
                           typename CONFIG_T::bias_t biases[CONFIG_T::n_filt]) {
    #pragma HLS INLINE
...
InitData:
    for (int id = 0; id < CONFIG_T::n_chan/data_T::size; id++) {
        #pragma HLS UNROLL
        data_T data_pack;
        data_pack = data_stream[id].read();
        for (int ii = 0; ii < data_T::size; ii++) {
            #pragma HLS UNROLL
            data[ii + id * data_T::size] = data_pack[ii];
        }
    }
...
CastLoop:
    for (unsigned jj = 0; jj < CONFIG_T::n_filt/res_T::size; jj++) {
        #pragma HLS UNROLL
        res_T res_pack;
        for (int ii = 0; ii < res_T::size; ii++) {
            #pragma HLS UNROLL
            res_pack[ii] = res[jj * res_T::size + ii];
        }
        res_stream[jj].write(res_pack);
    }
}
